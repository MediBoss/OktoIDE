...
class CreateFileController: UIViewController{
    ...
    @objc fileprivate func saveButtonTapped(sender: UIButton){
        ...
        guard let fileExtension = fileExtensionTextField.text else { return }

        let filename = "\(fileNameTextField.text ?? "").\(fileExtension)"
        
        GithubService.shared.crudFileContent(operation: .create, route: .createFile(projectName: project.name, fileName: filename), body: .createFile(commitMessage: "Create new file", content: ""), method: .put) { (result) in
